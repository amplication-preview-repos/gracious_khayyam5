...
const service = {
  ...
  solarFlares: () => FIND_MANY_RESULT,
  solarFlare: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("SolarFlare", () => {
  ...
  test("GET /solarFlares/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/solarFlares"}/${nonExistingId}`)
  ...
  test("GET /solarFlares/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/solarFlares"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /solarFlares existing resource", async () => {
